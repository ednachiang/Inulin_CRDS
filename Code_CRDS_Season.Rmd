...
### Calculate AUC - For Repeated Measures Test
```{r}
ids <- unique(crds.rem$ID)
rm.df <- data.frame()
...
### Subset by Substrate & ABX
rm.inu <- rm.df[which(rm.df$Substrate == "Inulin"),]
rm.sal <- rm.df[which(rm.df$Substrate == "Saline"),]
rm.inu.s <- rm.inu[which(rm.inu$Season == "Summer"),]
rm.inu.w <- rm.inu[which(rm.inu$Season == "Winter"),]
rm.inu.p <- rm.inu[which(rm.inu$Season == "Spring"),]
rm.sal.s <- rm.sal[which(rm.sal$Season == "Summer"),]
rm.sal.w <- rm.sal[which(rm.sal$Season == "Winter"),]
rm.sal.p <- rm.sal[which(rm.sal$Season == "Spring"),]
rm.inu.abx <- rm.inu[which(rm.inu$Antibiotics == "Yes"),]
rm.inu.noabx <- rm.inu[which(rm.inu$Antibiotics == "No"),]
rm.sal.abx <- rm.sal[which(rm.sal$Antibiotics == "Yes"),]
rm.sal.noabx <- rm.sal[which(rm.sal$Antibiotics == "No"),]
```
```



### Try Friedman test (non-parametric version of repeated measures ANOVA)
```{r}
# https://www.datanovia.com/en/lessons/friedman-test-in-r/

friedman.test(formula = as.numeric(AUC) ~ Season | BinTime, data = auc.inu.noabx)

friedman.test(y = auc.inu.noabx$AUC, groups = factor(auc.inu.noabx$Season), blocks = factor(auc.inu.noabx$ID) )



friedman.test(Delta ~ Season | RelTime, data = rm.inu.noabx)
