...
inu.sal.sum.plot <- ggplot(inu.sal.sum.exp, aes(x=X_Mean, y=Y_Mean, color=Season)) +
  ...
  scale_y_continuous(breaks=c(0, 500, 1000, 1500, 2000, 2500, 3000)) + # can't set limits or else horizontal error bar whiskers disappear
  ...
  geom_errorbarh(aes(xmin = X_Mean-X_SE, xmax = X_Mean+X_SE)) +
  facet_grid(.~Substrate) +
  xlab("Time Since Gavage (hr)") +
  ylab(expression("Mean"~~delta^13~"C"~"(%"[o]~")")) +
  scale_y_continuous(breaks=c(0, 500, 1000, 1500, 2000, 2500, 3000), limits=c(-30, 3050)) +
  scale_x_continuous(breaks=c(0, 1, 2, 3, 4), limits=c(-0.6, 4.5)) +
  theme(axis.text.x = element_text(size=16),
        axis.text.y = element_text(size=16),
        axis.title.x = element_text(size=20, margin=margin(t=10, r=0, b=0, l=0)),
...
### Mark baseline measurements to be plotted as dot, not a connected line
inu.sal.sum$Graph <- "Line"
inu.sal.sum$Graph[which(inu.sal.sum$BinTime < 0)] <- "Base"
  # Baseline measurements
inu.sal.sum$Graph <- factor(inu.sal.sum$Graph, levels=c("Base", "Line"))

# Plot Inulin & Saline CRDS curves
# Using X_Mean instead of BinTime because error bars don't align in BinTime
inu.sal.plot <- ggplot(inu.sal.sum, aes(x=X_Mean, y=Y_Mean, color=Season)) +
  geom_line(aes(linetype=Antibiotics, group = Graph), size=1) +
