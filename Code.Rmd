...
```{r}
cor(dna.sum$AUC, dna.sum$DNA_Yield, method="kendall")
  # 0.4118563

cor(dna.sum$MaxDelta, dna.sum$DNA_Yield, method="kendall")
  # 0.3885437

cor(dna.sum$PeakTime, dna.sum$DNA_Yield, method="kendall")
  # 0.4554219
```
...
# Remove taxa < 1% rel abund in a sample
physeq.ra <- transform_sample_counts(physeq.keep, function(x){x/sum(x)})
otu_table(physeq.ra)[otu_table(physeq.ra)<.01] <- 0
physeq.use <- prune_taxa(taxa_sums(physeq.ra)>0,physeq.ra)
