...
# Remove S34 (4007) because it's an outlier
crds.rem <- crds.rem[-which(crds.rem$ID == "S34"),]
# Remove S14 (4044) because it's an outlier
crds.rem <- crds.rem[-which(crds.rem$ID == "S14"),]
# Remove P04 because it's an adult
...
# Summarize
crds.sum <- crds.rem %>%
  group_by(Substrate, Antibiotics, Season, Diet, BinTime) %>%
...
p.adjust(c(0.005841, 0.01802, 0.01802, 0.002309, 0.2808, 0.01802, 0.2235, 0.5141, 0.0888), method = "BH")
  # Inulin = 0.0262845, 0.0324360, 0.0324360
  # Mannitol = 0.0207810, 0.3159000, 0.0324360
  # Saline = 0.2873571, 0.5141000, 0.1332000
...
p.adjust(c(0.002165,0.09732,0.00116,0.000666,0.149,0.001166,0.001166,0.09732,0.002331), method = "BH")
  # Inulin = 0.0034965 0.1094850 0.0026235
  # Mannitol = 0.0026235 0.1490000 0.0026235
  # Saline = 0.0026235 0.1094850 0.0034965
...
wilcox.test(AUC ~ Diet, data = auc.inu.s)
wilcox.test(AUC ~ Diet, data = auc.inu.w)
wilcox.test(AUC ~ Diet, data = auc.inu.p)
wilcox.test(AUC ~ Diet, data = auc.inu.noabx)
  # p-value = 0.968

summary(aov(formula = AUC ~ Season, data = auc.inu.noabx))
  # p-value = 0.243
TukeyHSD(aov(formula = AUC ~ Season, data = auc.inu.noabx))
  # Summer-Spring = 0.0629585
  # Winter-Spring = 0.0202059
  # Winter-Summer = 0.3432360

summary(aov(formula = AUC ~ Season, data = auc.man.noabx))
  # p-value = 0.761
...
ggplot(auc.noabx, aes(x = Season, y = AUC, fill = Season)) +
  ...
  scale_fill_manual(values = c("#e60000", "#5876d1","chartreuse3"), labels = c("Summer", "Winter", "Spring")) +
