...
### Calculate AUC after 1 h
```{r}
### Calculate AUC
ids <- levels(crds.auc$ID)
auc.beg1.df <- data.frame(crds.auc[1,])
auc.beg1.df <- auc.beg1.df[,-2:-3]
auc.beg1.df <- auc.beg1.df[,-8]
colnames(auc.beg1.df) <- c("AUC", "Number", "ID", "Season", "Sex", "Antibiotics", "Substrate")
auc.end1.df <- data.frame(crds.auc[1,])
auc.end1.df <- auc.end1.df[,-2:-3]
auc.end1.df <- auc.end1.df[,-8]
colnames(auc.end1.df) <- c("AUC", "Number", "ID", "Season", "Sex", "Antibiotics", "Substrate")
for(i in 1:length(ids)){
  ID <- ids[i]
  select <- crds.auc[which(crds.auc$ID == ID),]
  baseline <- which(select$RelTime < 0)
  ...
  baseline.co2 <- mean(select$CO2[baseline])
  baseline.bintime <- mean(select$BinTime[baseline])
  select.norm <- data.frame(RelTime=baseline.reltime, Delta=0,
                            CO2=baseline.co2, Number=select$Number[1],
                            ID=select$ID[1], Season=select$Season[1],
                            Sex=select$Sex[1], 
                            Antibiotics=select$Antibiotics[1],
                            Substrate=select$Substrate[1], 
                            BinTime=baseline.bintime)
    # Delta = 0 because we want to normalize everything to baseline. Any neg Deltas will decrease the AUC (subtracted from AUC), which is not what we want.
  exp <- which(select$RelTime>0)
  for(j in 1:(nrow(select)-length(baseline))){
    select.norm[j+1,] <- select[exp[j],]
  }
  select.norm[2:nrow(select.norm),2] <-
    (select.norm[2:nrow(select.norm),2]-baseline.delta)
  x <- select.norm$RelTime
  y <- select.norm$Delta
  #x2 <- x[which(x > 1.999)]
  #y2 <- y[which(x > 1.999)]
  #x3 <- x[which(x < 2)]
  #y3 <- y[which(x < 2)]
  AUC <- auc(x, y, from = 1, to = max(x), type='spline')
  #AUC <- auc(x2, y2, type = 'spline')
  auc.end1.df[i,1] <- AUC
  auc.end1.df[i,2:7] <- select.norm[1,4:9]
  AUC <- auc(x, y, from = min(x), to = 0.999999, type='spline')
  #AUC <- auc(x3, y3, type = 'spline')
  auc.beg1.df[i,1] <- AUC
  auc.beg1.df[i, 2:7] <- select.norm[1,4:9]
}
...
### Check Normality
hist(auc.beg1.df$AUC)
qqnorm(auc.beg1.df$AUC)
qqline(auc.beg1.df$AUC)
shapiro.test(auc.beg1.df$AUC)
  # p-value = 6.333e-12
hist(auc.end1.df$AUC)
qqnorm(auc.end1.df$AUC)
qqline(auc.end1.df$AUC)
shapiro.test(auc.end1.df$AUC)
  # p-value = 3.35e-15

### Boxplots
ggplot(auc.beg1.df, aes(x=Season, y=AUC, fill=Season)) +
  geom_boxplot(position=position_dodge(1)) +
  geom_point() +
  facet_grid(Antibiotics~Substrate)
ggplot(auc.end1.df, aes(x=Season, y=AUC, fill=Season)) +
  geom_boxplot(position=position_dodge(1)) +
  geom_point() +
  facet_grid(Antibiotics~Substrate)

### Separate
auc.inu.beg1 <- auc.beg1.df[which(auc.beg1.df$Substrate == "Inulin"),]
auc.inu.beg1.abx <- auc.inu.beg1[which(auc.inu.beg1$Antibiotics == "Yes"),]
auc.inu.beg1.noabx <- auc.inu.beg1[which(auc.inu.beg1$Antibiotics == "No"),]
auc.man.beg1 <- auc.beg1.df[which(auc.beg1.df$Substrate == "Mannitol"),]
auc.man.beg1.abx <- auc.man.beg1[which(auc.man.beg1$Antibiotics == "Yes"),]
auc.man.beg1.noabx <- auc.man.beg1[which(auc.man.beg1$Antibiotics == "No"),]
auc.inu.end1 <- auc.end1.df[which(auc.end1.df$Substrate == "Inulin"),]
auc.inu.end1.abx <- auc.inu.end1[which(auc.inu.end1$Antibiotics == "Yes"),]
auc.inu.end1.noabx <- auc.inu.end1[which(auc.inu.end1$Antibiotics == "No"),]
auc.man.end1 <- auc.end1.df[which(auc.end1.df$Substrate == "Mannitol"),]
auc.man.end1.abx <- auc.man.end1[which(auc.man.end1$Antibiotics == "Yes"),]
auc.man.end1.noabx <- auc.man.end1[which(auc.man.end1$Antibiotics == "No"),]


# Inulin
wilcox.test(AUC~Antibiotics, data=auc.inu.beg1)
  # p-value = 0.05204
kruskal.test(formula = AUC ~ Season, data=auc.inu.beg1.abx)
  # p-value = 0.4647
kruskal.test(formula = AUC ~ Season, data=auc.inu.beg1.noabx)
  # p-value = 0.08275
kruskalmc(resp = AUC ~ Season, data=auc.inu.beg1.noabx)
  # Spring-Winter
pairwise.wilcox.test(auc.inu.beg1.noabx$AUC, auc.inu.beg1.noabx$Season)
  # Summer-Spring = 0.091
  # Winter-Spring = 0.530
  # Summer-Winter = 0.444
wilcox.test(AUC~Antibiotics, data=auc.inu.beg1)
  # p-value = 0.05204
kruskal.test(formula = AUC ~ Season, data=auc.inu.end1.abx)
  # p-value = 0.3472
kruskal.test(formula = AUC ~ Season, data=auc.inu.end1.noabx)
  # p-value = 0.09011
kruskalmc(resp = AUC ~ Season, data=auc.inu.end1.noabx)
  # Spring-Winter
pairwise.wilcox.test(auc.inu.end1.noabx$AUC, auc.inu.end1.noabx$Season)
  # Summer-Spring = 0.030
  # Winter-Spring = 0.030
  # Summer-Winter = 0.29
...
# Mannitol
wilcox.test(AUC~Antibiotics, data=auc.man.beg1)
  # p-value = 0.3851
kruskal.test(formula = AUC ~ Season, data=auc.man.beg1.abx)
  # p-value = 0.3798
kruskal.test(formula = AUC ~ Season, data=auc.man.beg1.noabx)
  # p-value = 0.7867
kruskalmc(resp = AUC ~ Season, data=auc.man.beg1.noabx)
  # Spring-Winter
pairwise.wilcox.test(auc.man.beg1.noabx$AUC, auc.man.beg1.noabx$Season)
  # Summer-Spring = 1
  # Winter-Spring = 1
  # Summer-Winter = 1
wilcox.test(AUC~Antibiotics, data=auc.man.end1)
  # p-value = 4.395e-05
kruskal.test(formula = AUC ~ Season, data=auc.man.end1.abx)
  # p-value = 0.4642
kruskal.test(formula = AUC ~ Season, data=auc.man.end1.noabx)
  # p-value = 0.3005
kruskalmc(resp = AUC ~ Season, data=auc.man.end1.noabx)
  # Spring-Winter
pairwise.wilcox.test(auc.man.end1.noabx$AUC, auc.man.end1.noabx$Season)
  # Summer-Spring = 1
  # Winter-Spring = 1
  # Summer-Winter = 0.3


### Kruskal-Wallis BH-adjust
p.adjust(c(0.4647, 0.08275, 0.3472, 0.09011, 0.3798, 0.7867, 0.4642, 0.3005), method="BH")
  # Inulin Beg ABX Season = 0.5310857
  # Inulin Beg No ABX Season = 0.3604400
  # Inulin End ABX Season = 0.5310857
  # Inulin End No ABX Season = 0.3604400
  # Mannitol Beg ABX Season = 0.5310857
  # Mannitol Beg No ABX Season = 0.7867000
  # Mannitol End ABX Season = 0.5310857
  # Mannitol End No ABX Season = 0.5310857
```
### Calculate AUC after 2 h
```{r}
### Calculate AUC
ids <- levels(crds.auc$ID)
auc.beg2.df <- data.frame(crds.auc[1,])
auc.beg2.df <- auc.beg2.df[,-2:-3]
auc.beg2.df <- auc.beg2.df[,-8]
colnames(auc.beg2.df) <- c("AUC", "Number", "ID", "Season", "Sex", "Antibiotics", "Substrate")
auc.end2.df <- data.frame(crds.auc[1,])
auc.end2.df <- auc.end2.df[,-2:-3]
auc.end2.df <- auc.end2.df[,-8]
colnames(auc.end2.df) <- c("AUC", "Number", "ID", "Season", "Sex", "Antibiotics", "Substrate")
for(i in 1:length(ids)){
  ID <- ids[i]
  select <- crds.auc[which(crds.auc$ID == ID),]
  baseline <- which(select$RelTime < 0)
  ...
  #x3 <- x[which(x < 2)]
  #y3 <- y[which(x < 2)]
  AUC <- auc(x, y, from = 2, to = max(x), type='spline')
  #AUC <- auc(x2, y2, type = 'spline')
  auc.end2.df[i,1] <- AUC
  auc.end2.df[i,2:7] <- select.norm[1,4:9]
  AUC <- auc(x, y, from = min(x), to = 1.999999, type='spline')
  #AUC <- auc(x3, y3, type = 'spline')
  auc.beg2.df[i,1] <- AUC
  auc.beg2.df[i, 2:7] <- select.norm[1,4:9]
}
...
### Check Normality
hist(auc.beg2.df$AUC)
qqnorm(auc.beg2.df$AUC)
qqline(auc.beg2.df$AUC)
shapiro.test(auc.beg2.df$AUC)
  # p-value = 2.864e-14
hist(auc.end2.df$AUC)
qqnorm(auc.end2.df$AUC)
qqline(auc.end2.df$AUC)
shapiro.test(auc.end2.df$AUC)
  # p-value = 8.118e-16

### Boxplots
ggplot(auc.beg2.df, aes(x=Season, y=AUC, fill=Season)) +
  geom_boxplot(position=position_dodge(1)) +
  geom_point() +
  facet_grid(Antibiotics~Substrate)
ggplot(auc.end2.df, aes(x=Season, y=AUC, fill=Season)) +
  geom_boxplot(position=position_dodge(1)) +
  geom_point() +
  facet_grid(Antibiotics~Substrate)

### Separate
auc.inu.beg2 <- auc.beg2.df[which(auc.beg2.df$Substrate == "Inulin"),]
auc.inu.beg2.abx <- auc.inu.beg2[which(auc.inu.beg2$Antibiotics == "Yes"),]
auc.inu.beg2.noabx <- auc.inu.beg2[which(auc.inu.beg2$Antibiotics == "No"),]
auc.man.beg2 <- auc.beg2.df[which(auc.beg2.df$Substrate == "Mannitol"),]
auc.man.beg2.abx <- auc.man.beg2[which(auc.man.beg2$Antibiotics == "Yes"),]
auc.man.beg2.noabx <- auc.man.beg2[which(auc.man.beg2$Antibiotics == "No"),]
auc.inu.end2 <- auc.end2.df[which(auc.end2.df$Substrate == "Inulin"),]
auc.inu.end2.abx <- auc.inu.end2[which(auc.inu.end2$Antibiotics == "Yes"),]
auc.inu.end2.noabx <- auc.inu.end2[which(auc.inu.end2$Antibiotics == "No"),]
auc.man.end2 <- auc.end2.df[which(auc.end2.df$Substrate == "Mannitol"),]
auc.man.end2.abx <- auc.man.end2[which(auc.man.end2$Antibiotics == "Yes"),]
auc.man.end2.noabx <- auc.man.end2[which(auc.man.end2$Antibiotics == "No"),]

# Inulin
wilcox.test(AUC~Antibiotics, data=auc.inu.beg2)
  # p-value = 0.011
kruskal.test(formula = AUC ~ Season, data=auc.inu.beg2.abx)
  # p-value = 0.6015
kruskal.test(formula = AUC ~ Season, data=auc.inu.beg2.noabx)
  # p-value = 0.8821
kruskalmc(resp = AUC ~ Season, data=auc.inu.beg2.noabx)
  # Spring-Winter
pairwise.wilcox.test(auc.inu.beg2.noabx$AUC, auc.inu.beg2.noabx$Season)
  # Summer-Spring = 1
  # Winter-Spring = 1
  # Summer-Winter = 1
wilcox.test(AUC~Antibiotics, data=auc.inu.end2)
  # p-value = 7.112e-06
kruskal.test(formula = AUC ~ Season, data=auc.inu.end2.abx)
  # p-value = 0.0758
kruskal.test(formula = AUC ~ Season, data=auc.inu.end2.noabx)
  # p-value = 0.02651
kruskalmc(resp = AUC ~ Season, data=auc.inu.end2.noabx)
  # Spring-Winter
pairwise.wilcox.test(auc.inu.end2.noabx$AUC, auc.inu.end2.noabx$Season, p.adjust.method="BH")
  # Summer-Spring = 0.298
  # Winter-Spring = 0.024
  # Summer-Winter = 0.298

# Mannitol
wilcox.test(AUC~Antibiotics, data=auc.man.beg)
  # p-value = 0.0002716
kruskal.test(formula = AUC ~ Season, data=auc.man.beg2.abx)
  # p-value = 0.7697
kruskal.test(formula = AUC ~ Season, data=auc.man.beg2.noabx)
  # p-value = 0.3315
kruskalmc(resp = AUC ~ Season, data=auc.man.beg2.noabx)
  # Spring-Winter
pairwise.wilcox.test(auc.man.beg2.noabx$AUC, auc.man.beg2.noabx$Season)
  # Summer-Spring = 1
  # Winter-Spring = 1
  # Summer-Winter = 0.41
wilcox.test(AUC~Antibiotics, data=auc.man.end2)
  # p-value = 4.395e-05
kruskal.test(formula = AUC ~ Season, data=auc.man.end2.abx)
  # p-value = 0.01283
kruskal.test(formula = AUC ~ Season, data=auc.man.end2.noabx)
  # p-value = 0.5263
kruskalmc(resp = AUC ~ Season, data=auc.man.end2.noabx)
  # Spring-Winter
pairwise.wilcox.test(auc.man.end2.noabx$AUC, auc.man.end2.noabx$Season)
  # Summer-Spring = 1
  # Winter-Spring = 1
  # Summer-Winter = 0.88
...
### Kruskal-Wallis BH-Adjust
p.adjust(c(0.6015, 0.8821, 0.0758, 0.02651, 0.7697, 0.3315, 0.01283, 0.5263), method="BH")
  # Inulin Beg ABX Season = 0.8020000
  # Inulin Beg No ABX Season = 0.8821000
  # Inulin End ABX Season = 0.2021333
  # Inulin End No ABX Season = 0.1060400
  # Mannitol Beg ABX Season = 0.8796571
  # Mannitol Beg No ABX Season = 0.6630000
  # Mannitol End ABX Season = 0.1026400
  # Mannitol End No ABX Season = 0.8020000
...
tiff("crds.mdtp.tiff", width = 12, height = 3.75, units = "in", res = 300)
carb.plot
dev.off()
```
...
tiff("inu.man.mdtp.tiff", width = 12, height = 3.25, units = "in", res = 300)
grid.newpage()
pushViewport(viewport(layout=(grid.layout(1,3, widths=c(1,1,0.2), heights=c(1,1,1)))))
print(inu.plot, vp=viewport(layout.pos.row=1, layout.pos.col=1))
print(man.plot, vp=viewport(layout.pos.row=1, layout.pos.col=2))
