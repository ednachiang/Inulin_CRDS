...
# Alpha Div
adiv <- data.frame(sample_data(physeq.keep))

# OTU Table
otus <- data.frame(otu_table(physeq.keep))
...
top5.auc.df <- data.frame(tax_table(physeq.keep))[1,2:7]
bot5.auc.df <- data.frame(tax_table(physeq.keep))[1,2:7]
for(i in 1:5){
  OTU <- top5.auc[i]
  pick <- which(data.frame(tax_table(physeq.keep))$Species == OTU)
  taxon <- data.frame(tax_table(physeq.keep))[pick,]
...
top5.del.df <- data.frame(tax_table(physeq.keep))[1,2:7]
bot5.del.df <- data.frame(tax_table(physeq.keep))[1,2:7]
for(i in 1:5){
  OTU <- top5.del[i]
  pick <- which(data.frame(tax_table(physeq.keep))$Species == OTU)
  taxon <- data.frame(tax_table(physeq.keep))[pick,]
...
top5.tim.df <- data.frame(tax_table(physeq.keep))[1,2:7]
bot5.tim.df <- data.frame(tax_table(physeq.keep))[1,2:7]
for(i in 1:5){
  OTU <- top5.tim[i]
  pick <- which(data.frame(tax_table(physeq.keep))$Species == OTU)
  taxon <- data.frame(tax_table(physeq.keep))[pick,]
...
xyplot(otus$Otu00109 ~ dna.keep$AUC)
  # Damn that's one crazy outlier
summary(lm(otus$Otu00109 ~ dna.keep$AUC))
  # Outlier is definitely driving the significance
  # p-value = 0.000122
  # R2 = 0.5448
confint(lm(otus$Otu00109 ~ dna.keep$AUC), level=0.95)
rownames(otus)[which(otus$Otu00109 == max(otus$Otu00109))]
