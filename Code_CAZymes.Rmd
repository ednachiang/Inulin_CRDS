...
clust.test <- pvclust(t(hell), method.hclust = "average", method.dist = "euclidean", nboot = 1000)
plot(clust.test)
pvrect(clust.test, alpha = 0.95)

# Remove S.W20 and S.W37 because they seem to be outliers

clust.df <- t(hell)
clust.df <- clust.df[,-18]
clust.df <- clust.df[,-16]
clust.test2 <- pvclust(clust.df, method.hclust = "average", method.dist = "euclidean", nboot = 1000)
plot(clust.test2)
pvrect(clust.test2, alpha = 0.95)
```
