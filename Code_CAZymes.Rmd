...
# Fligner P Adj
p.adjust(c(0.993, 0.7951, 0.4993, 0.04577), "BH")
  # CE = 0.99300
  # GH = 0.99300
  # GT = 0.99300
  # PL = 0.18308
...
gh.plot <- ggplot(class.df[which(class.df$OTU == "Glycoside Hydrolase"),], 
                   aes(x=Season, y=Abundance, fill=Season)) +
  geom_dotplot(binaxis='y', stackdir='center', dotsize=2, stackratio=1.2) +
...
### Plot - Class dotplot
```{r}
#tiff("cazy.class.tiff", width = 6.5, height = 4.5, units = "in", res = 600)
grid.newpage()
pushViewport(viewport(layout=grid.layout(2,3, widths = c(1,1, 0.35))))

print(gh.plot, vp=viewport(layout.pos.row=1, layout.pos.col=1))
grid.text("A", x=unit(0.05, "npc"), y=unit(0.95, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=14,fontface="bold"))


print(gt.plot, vp=viewport(layout.pos.row=1, layout.pos.col=2))
grid.text("B", x=unit(0.05, "npc"), y=unit(0.95, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=2), gp=gpar(fontsize=14,fontface="bold"))


print(ce.plot, vp=viewport(layout.pos.row=2, layout.pos.col=1))
grid.text("C", x=unit(0.05, "npc"), y=unit(0.95, "npc"), vp=viewport(layout.pos.row=2, layout.pos.col=1), gp=gpar(fontsize=14,fontface="bold"))



print(pl.plot, vp=viewport(layout.pos.row=2, layout.pos.col=2))
grid.text("D", x=unit(0.05, "npc"), y=unit(0.95, "npc"), vp=viewport(layout.pos.row=2, layout.pos.col=2), gp=gpar(fontsize=14,fontface="bold"))


season.legend$vp <- viewport(layout.pos.row=1:2, layout.pos.col=3)
...
hm.active <- plot_heatmap(sig.active, sample.label = "Season", sample.order = c("I.S07", "S.S08", "I.S29", "S.S32",
  ...
  guides(fill = guide_colorbar(barheight = unit(11,"in"),
                               frame.colour="black",
                               frame.linewidth=1))
#dev.off()
...
hm.active.high <- plot_heatmap(sig.active.high, sample.label = "Season", sample.order = c("I.S07", "S.S08", "I.S29", 
                                                                        "S.S32", "I.S34", "S.S05",
                                                                        "S.W36", "I.W32", "I.W31",
                                                                        "I.W33",  "S.W37", "I.P01",
                                                                        "S.P03", "I.P07", "S.P09",
  ...
  scale_fill_gradientn(name=expression("Hellinger-\ntransformed\nnormalized\ncounts"),
                       colours = c("#FFFFFF", "#FDF8F3", "#f9e7d7", "#f6c6a9", "#f49c74", "#d9444f", "#7b1c59", "#2e1438", "#000000"),
                       limits = c(0.156, 0.244), breaks = c(0.156, 0.200, 0.244),
                       labels = c("0.156", "0.200", "0.244")) +
  theme(axis.text.x = element_blank(),
        axis.text.y = element_text(size=30),
        axis.ticks.x = element_blank(),
        axis.ticks.y = element_blank(),
        ...
        plot.margin = margin(t=65, r=5, b=85, l=15)) +
  guides(fill = guide_colorbar(barheight = unit(1.75,"in"),
                               frame.colour="black",
                               frame.linewidth=1))
#dev.off()
...
hm.hib <- plot_heatmap(sig.hib, sample.label = "Season", sample.order = c("I.S07", "S.S08", "I.S29","S.S32",
                                                                "I.S34", "S.S05", "S.W36", "I.W32",
                                                                "I.W31", "I.W33",  "S.W37", "I.P01",
                                                                "S.P03", "I.P07", "S.P09", "I.P11",
                                                                "S.P12"), 
  ...
  guides(fill = guide_colorbar(barheight = unit(5.75,"in"),
                               frame.colour="black",
                               frame.linewidth=1))
#dev.off()
...
### Plot - Heatmaps
```{r}
tiff("cazy.heatmaps.tiff", width=18, height=28, units="in", res=300)
grid.newpage()
pushViewport(viewport(layout=grid.layout(3,1, heights = c(1,0.35,0.6))))


print(hm.active, vp=viewport(layout.pos.row=1, layout.pos.col=1))
grid.text("A", x=unit(0.02, "npc"), y=unit(0.96, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=50,fontface="bold"))

grid.rect(x=unit(0.278, "npc"), y=unit(0.934, "npc"), width=unit(0.245, "npc"),
          height=unit(0.06,"npc"), draw = T, 
          vp=viewport(layout.pos.row=1, layout.pos.col=1),
          gp=gpar(fill = "#E69F00", color = "#E69F00"))
grid.rect(x=unit(0.5045, "npc"), y=unit(0.934, "npc"), width=unit(0.203, "npc"),
          height=unit(0.06,"npc"), draw = T, 
          vp=viewport(layout.pos.row=1, layout.pos.col=1),
          gp=gpar(fill = "#0072B2", color = "#0072B2"))
grid.rect(x=unit(0.731, "npc"), y=unit(0.934, "npc"), width=unit(0.245, "npc"),
          height=unit(0.06,"npc"), draw = T, 
          vp=viewport(layout.pos.row=1, layout.pos.col=1),
          gp=gpar(fill = "#00bf8b", color = "#00bf8b"))

grid.text("Summer", x=unit(0.278, "npc"), y=unit(0.935, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=40))
grid.text("Winter", x=unit(0.505, "npc"), y=unit(0.935, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=40))
grid.text("Spring", x=unit(0.731, "npc"), y=unit(0.935, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=40))

grid.text("ab", x=unit(0.05, "npc"), y=unit(0.879, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("ab", x=unit(0.05, "npc"), y=unit(0.818, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("ab", x=unit(0.05, "npc"), y=unit(0.76, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("ab", x=unit(0.05, "npc"), y=unit(0.705, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("ab", x=unit(0.05, "npc"), y=unit(0.65, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("ab", x=unit(0.05, "npc"), y=unit(0.595, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("ab", x=unit(0.05, "npc"), y=unit(0.535, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("ab", x=unit(0.05, "npc"), y=unit(0.479, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("ab", x=unit(0.05, "npc"), y=unit(0.425, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("ab", x=unit(0.05, "npc"), y=unit(0.37, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("ab", x=unit(0.05, "npc"), y=unit(0.315, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("b", x=unit(0.0565, "npc"), y=unit(0.26, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("b", x=unit(0.0565, "npc"), y=unit(0.204, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("b", x=unit(0.0565, "npc"), y=unit(0.146, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("b", x=unit(0.0565, "npc"), y=unit(0.09, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("b", x=unit(0.0565, "npc"), y=unit(0.035, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=28))
...
print(hm.active.high, vp=viewport(layout.pos.row=2, layout.pos.col=1))
grid.text("B", x=unit(0.02, "npc"), y=unit(0.85, "npc"), vp=viewport(layout.pos.row=2, layout.pos.col=1), gp=gpar(fontsize=50,fontface="bold"))

grid.rect(x=unit(0.277, "npc"), y=unit(0.625, "npc"), width=unit(0.246, "npc"),
          height=unit(0.16,"npc"), draw = T, 
          vp=viewport(layout.pos.row=2, layout.pos.col=1),
          gp=gpar(fill = "#E69F00", color = "#E69F00"))
grid.rect(x=unit(0.506, "npc"), y=unit(0.625, "npc"), width=unit(0.207, "npc"),
          height=unit(0.16,"npc"), draw = T, 
          vp=viewport(layout.pos.row=2, layout.pos.col=1),
          gp=gpar(fill = "#0072B2", color = "#0072B2"))
grid.rect(x=unit(0.735, "npc"), y=unit(0.625, "npc"), width=unit(0.247, "npc"),
          height=unit(0.16,"npc"), draw = T, 
          vp=viewport(layout.pos.row=2, layout.pos.col=1),
          gp=gpar(fill = "#00bf8b", color = "#00bf8b"))

grid.text("Summer", x=unit(0.277, "npc"), y=unit(0.63, "npc"), vp=viewport(layout.pos.row=2, layout.pos.col=1), gp=gpar(fontsize=40))
grid.text("Winter", x=unit(0.506, "npc"), y=unit(0.63, "npc"), vp=viewport(layout.pos.row=2, layout.pos.col=1), gp=gpar(fontsize=40))
grid.text("Spring", x=unit(0.735, "npc"), y=unit(0.63, "npc"), vp=viewport(layout.pos.row=2, layout.pos.col=1), gp=gpar(fontsize=40))

grid.text("ab", x=unit(0.061, "npc"), y=unit(0.47, "npc"), vp=viewport(layout.pos.row=2, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("a", x=unit(0.055, "npc"), y=unit(0.33, "npc"), vp=viewport(layout.pos.row=2, layout.pos.col=1), gp=gpar(fontsize=28))
...
print(hm.hib, vp=viewport(layout.pos.row=3, layout.pos.col=1))
grid.text("C", x=unit(0.02, "npc"), y=unit(0.95, "npc"), vp=viewport(layout.pos.row=3, layout.pos.col=1), gp=gpar(fontsize=50,fontface="bold"))

grid.rect(x=unit(0.279, "npc"), y=unit(0.889, "npc"), width=unit(0.248, "npc"),
          height=unit(0.1,"npc"), draw = T, 
          vp=viewport(layout.pos.row=3, layout.pos.col=1),
          gp=gpar(fill = "#E69F00", color = "#E69F00"))
grid.rect(x=unit(0.509, "npc"), y=unit(0.889, "npc"), width=unit(0.207, "npc"),
          height=unit(0.1,"npc"), draw = T, 
          vp=viewport(layout.pos.row=3, layout.pos.col=1),
          gp=gpar(fill = "#0072B2", color = "#0072B2"))
grid.rect(x=unit(0.739, "npc"), y=unit(0.889, "npc"), width=unit(0.248, "npc"),
          height=unit(0.1,"npc"), draw = T, 
          vp=viewport(layout.pos.row=3, layout.pos.col=1),
          gp=gpar(fill = "#00bf8b", color = "#00bf8b"))

grid.text("Summer", x=unit(0.279, "npc"), y=unit(0.891, "npc"), vp=viewport(layout.pos.row=3, layout.pos.col=1), gp=gpar(fontsize=40))
grid.text("Winter", x=unit(0.509, "npc"), y=unit(0.891, "npc"), vp=viewport(layout.pos.row=3, layout.pos.col=1), gp=gpar(fontsize=40))
grid.text("Spring", x=unit(0.738, "npc"), y=unit(0.891, "npc"), vp=viewport(layout.pos.row=3, layout.pos.col=1), gp=gpar(fontsize=40))


grid.text("cd", x=unit(0.05, "npc"), y=unit(0.79, "npc"), vp=viewport(layout.pos.row=3, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("cd", x=unit(0.05, "npc"), y=unit(0.7, "npc"), vp=viewport(layout.pos.row=3, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("cd", x=unit(0.05, "npc"), y=unit(0.61, "npc"), vp=viewport(layout.pos.row=3, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("cd", x=unit(0.05, "npc"), y=unit(0.52, "npc"), vp=viewport(layout.pos.row=3, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("cd", x=unit(0.05, "npc"), y=unit(0.43, "npc"), vp=viewport(layout.pos.row=3, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("cd", x=unit(0.05, "npc"), y=unit(0.34, "npc"), vp=viewport(layout.pos.row=3, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("d", x=unit(0.056, "npc"), y=unit(0.25, "npc"), vp=viewport(layout.pos.row=3, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("d", x=unit(0.056, "npc"), y=unit(0.16, "npc"), vp=viewport(layout.pos.row=3, layout.pos.col=1), gp=gpar(fontsize=28))
grid.text("d", x=unit(0.056, "npc"), y=unit(0.07, "npc"), vp=viewport(layout.pos.row=3, layout.pos.col=1), gp=gpar(fontsize=28))
dev.off()
```
...
#removeTaxa('kaiju/sig_cazymes/relAbund/')
...
fileList <- list.files('kaiju/sig_cazymes/relAbund/rem/test/', pattern = ".csv")
  # List of all .csv files
for(file in 1:length(fileList)){
  fileImport <- read.csv(paste("kaiju/sig_cazymes/relAbund/rem/test/", fileList[file], sep=""), header=T, row.names=2)
  fileImport <- fileImport[,-1]
  print(fileList[file])
  
  # Create OTU table
  file.otu <- otu_table(fileImport, taxa_are_rows = T)
  
  # Create tax table
  taxa.names <- data.frame(full = taxa_names(file.otu))
  taxa.names <- cbind(taxa.names, colsplit(taxa.names$full, pattern = ";", names = c("Domain", "Phylum", "Class", "Order", "Family", "Genus", "Species")))
  rownames(taxa.names) <- taxa.names$full
  taxa.names <- taxa.names[,-1]
  file.tax <- tax_table(taxa.names)
  
  # Make phyloseq object
  taxa_names(file.tax) <- taxa_names(file.otu)
  file.phy <- merge_phyloseq(file.tax, file.otu, meta.samp)
  print(file.phy)
  
  # Save physeq object
  fileName <- strsplit(fileList[file], split = "[.]")
  fileName <- unlist(fileName)
  newName <- paste("kaiju/sig_cazymes/relAbund/rem/test/", fileName[1], ".", fileName[2], ".RData", sep="")
  save(file.phy, file=newName)
}
```

### DON'T RUN - Taxa Summary
```{r}
fileList <- list.files('kaiju/sig_cazymes/relAbund/rem/physeq/', pattern = ".RData")

for(file in 1:length(fileList)){
  load(paste("kaiju/sig_cazymes/relAbund/rem/physeq/", fileList[file], sep=""))
    # output = file.phy
  print(fileList[file])
  
  physeq.melt <- psmelt(file.phy)
  physeqSum <- physeq.melt %>%
    group_by(Season, OTU) %>%
    summarize(Mean = mean(Abundance),
            SD = sd(Abundance),
            SE = se(Abundance))
  write.csv(physeqSum, file = paste("kaiju/sig_cazymes/relAbund/rem/physeq/", fileList[file],".summarized.table.csv", sep=""))
}
```