...
# Order seasons
auc.df$Season <- ordered(auc.df$Season, levels=c("Summer", "Winter", "Spring"))
auc.df$Antibiotics <- ordered(auc.df$Antibiotics, level=c("No", "Yes"))

# Calculate square root of AUC
auc.df$AUC_norm <- auc.df$AUC + (min(auc.df$AUC) * -1)
  # Since the min value is negative, normalize data so that min = 0
  # Square root can't be done on a negative value
auc.df$AUC_sqrt <- sqrt(auc.df$AUC_norm)
...
```{r}
### Boxplot - AUC
ggplot(auc.inu.s, aes(x = Antibiotics, y = AUC_sqrt, fill = Antibiotics)) +
  geom_boxplot()
  # 1 ABX
ggplot(auc.inu.w, aes(x = Antibiotics, y = AUC_sqrt, fill = Antibiotics)) +
  geom_boxplot()
ggplot(auc.inu.p, aes(x = Antibiotics, y = AUC_sqrt, fill = Antibiotics)) +
  geom_boxplot()
  # 2 ABX
ggplot(auc.sal.s, aes(x = Antibiotics, y = AUC_sqrt, fill = Antibiotics)) +
  geom_boxplot()
ggplot(auc.sal.w, aes(x = Antibiotics, y = AUC_sqrt, fill = Antibiotics)) +
  geom_boxplot()
  # 1 No ABX
  # 2 ABX
ggplot(auc.sal.p, aes(x = Antibiotics, y = AUC_sqrt, fill = Antibiotics)) +
  geom_boxplot()
