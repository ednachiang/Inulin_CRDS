...
kruskal.pretty = function(otu, metrics, csv, interesting, output_name, taxonomy){
  ...
  for(b in unique(csv$Comparison)){
    ...
    sample_L = metrics$ID[as.numeric(rownames(subset(metrics, metrics[[topic1]] == c(cola))))]
    sample_R = metrics$ID[as.numeric(rownames(subset(metrics, metrics[[topic1]] == c(colb))))]
    ...
    for(otus in otu_list$OTU){
      ...
      for(sample in sample_L){
        L=append(L,abund[which(rownames(abund) == otus), which(colnames(abund) == sample)])
        mean_L[[otus]]=mean(as.numeric(L))
        sd_L[[otus]]=sd(as.numeric(L))
      }
      for(sample in sample_R){
        R=append(R,abund[which(rownames(abund) == otus), which(colnames(abund) == sample)])
    ...
    for(otus in otu_list$OTU){
      otus = as.character(otus)
      result=kruskal.test(listbact[[which(otu_list$OTU == otus)]]~listmet[[topic1]])
