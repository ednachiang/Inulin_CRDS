...
### Prepare CRDS data
meta.crds <- read.csv('metadata.csv')

# Add metadata
crds.use[, colnames(meta.crds)[3:ncol(meta.crds)]] <- NA
for(i in 1:nrow(crds.use)){
  meta.crds.row <- which(meta.crds$ID == crds.use$ID[i])
  crds.use[i, 7:ncol(crds.use)] <- meta.crds[meta.crds.row, 3:ncol(meta.crds)]
}
...
crds.rem <- crds.rem[-which(crds.rem$ID == "W43"),]
crds.rem <- crds.rem[-which(crds.rem$ID == "W20"),]
  # This is due to odd meta.crdsgenome
crds.rem <- crds.rem[-which(crds.rem$ID == "W50"),]

# Add CRDS metrics to 16S metadata
meta <- meta[-which(meta$ID == "S11"),]
for (j in 1:nrow(meta)){
  useRow <- which(crds.use$ID == meta$ID[j])
  meta$Baseline[j] <- crds.use$Baseline[useRow]
  meta$AUC_sqrt[j] <- crds.use$AUC_sqrt[useRow]
  meta$AvgD[j] <- crds.use$AvgD[useRow]
  meta$MaxD[j] <- crds.use$MaxD[useRow]
  meta$MaxSlope[j] <- crds.use$MaxSlope[useRow]
}
```
