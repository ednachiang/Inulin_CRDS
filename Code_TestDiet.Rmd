...
crds.rem <- crds.bin[-which(crds.bin$ID == "P04"),]
...
# Inulin Summer ABX
wilcox.test(TimeMaxD ~ Diet, data = d.inu.s[which(d.inu.s$Antibiotics == "Yes"),])
  # p-value = 0.4
...
# Inulin Summer ABX
fligner.test(TimeMaxD ~ Diet, data = d.inu.s[which(d.inu.s$Antibiotics == "Yes"),])
  # p-value = 0.3381
...
# Inulin Summer ABX
wilcox.test(Slope ~ Diet, data = d.inu.s[which(d.inu.s$Antibiotics == "Yes"),])
  # p-value = 1
...
# Inulin Summer ABX
fligner.test(Slope ~ Diet, data = d.inu.s[which(d.inu.s$Antibiotics == "Yes"),])
  # p-value = 0.04413*
  # But it looks like there was an ABX male that didn't work, so I need to double check [DNA] and decide whether it's an outlier or not
