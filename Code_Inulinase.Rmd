...
phy.sum <- phy.melt %>%
  ...
  summarize(Mean = mean(Abundance),
            SD = sd(Abundance),
            SE = se(Abundance))
#write.csv(phy.sum, file = "kaiju/inulinase/tax_summarized/phylum.sum.csv", row.names = F)
...
cla.sum <- cla.melt %>%
  ...
  summarize(Mean = mean(Abundance),
            SD = sd(Abundance),
            SE = se(Abundance))
#write.csv(cla.sum, file = "kaiju/inulinase/tax_summarized/class.sum.csv", row.names = F)
...
ord.sum <- ord.melt %>%
  ...
  summarize(Mean = mean(Abundance),
            SD = sd(Abundance),
            SE = se(Abundance))
#write.csv(ord.sum, file = "kaiju/inulinase/tax_summarized/order.sum.csv", row.names = F)
...
fam.sum <- fam.melt %>%
  ...
  summarize(Mean = mean(Abundance),
            SD = sd(Abundance),
            SE = se(Abundance))
#write.csv(fam.sum, file = "kaiju/inulinase/tax_summarized/family.sum.csv", row.names = F)
...
gen.sum <- gen.melt %>%
  ...
  summarize(Mean = mean(Abundance),
            SD = sd(Abundance),
            SE = se(Abundance))
#write.csv(gen.sum, file = "kaiju/inulinase/tax_summarized/genus.sum.csv", row.names = F)
...
spe.sum <- spe.melt %>%
  ...
  summarize(Mean = mean(Abundance),
            SD = sd(Abundance),
            SE = se(Abundance))
#write.csv(spe.sum, file = "kaiju/inulinase/tax_summarized/species.sum.csv", row.names = F)
```
...
wil.phy <- data.frame()
for(taxon in 1:dim(data.frame(otu_table(phy.phy)))[1]){
  wil <- wilcox.test(x=as.numeric(otus[taxon,]), g=meta$Metabolism)
  wil.phy <- rbind(wil.phy, data.frame(taxon = rownames(otus)[taxon], pval = wil$p.value))
}
wil.phy$padj <- p.adjust(wil.phy$pval, method = "BH")
...
for(taxon in 1:dim(data.frame(otu_table(cla.phy)))[1]){
  wil <- wilcox.test(x=as.numeric(otus[taxon,]), g=meta$Metabolism)
  wil.cla <- rbind(wil.cla, data.frame(taxon = rownames(otus)[taxon], pval = wil$p.value))
}
wil.cla$padj <- p.adjust(wil.cla$pval, method = "BH")
...
for(taxon in 1:dim(data.frame(otu_table(ord.phy)))[1]){
  wil <- wilcox.test(x=as.numeric(otus[taxon,]), g=meta$Metabolism)
  wil.ord <- rbind(wil.ord, data.frame(taxon = rownames(otus)[taxon], pval = wil$p.value))
}
wil.ord$padj <- p.adjust(wil.ord$pval, method = "BH")
...
for(taxon in 1:dim(data.frame(otu_table(fam.phy)))[1]){
  wil <- wilcox.test(x=as.numeric(otus[taxon,]), g=meta$Metabolism)
  wil.fam <- rbind(wil.fam, data.frame(taxon = rownames(otus)[taxon], pval = wil$p.value))
}
wil.fam$padj <- p.adjust(wil.fam$pval, method = "BH")
...
for(taxon in 1:dim(data.frame(otu_table(gen.phy)))[1]){
  wil <- wilcox.test(x=as.numeric(otus[taxon,]), g=meta$Metabolism)
  wil.gen <- rbind(wil.gen, data.frame(taxon = rownames(otus)[taxon], pval = wil$p.value))
}
wil.gen$padj <- p.adjust(wil.gen$pval, method = "BH")
...
ggplot(phy.df, aes(x=Season, y=Abundance, fill=Season)) +
  geom_dotplot(binaxis='y', stackdir = "center", dotsize = 1.5) +
  facet_wrap(Phylum ~ ., ncol = 3, scales = "free", labeller = labeller(OTU = new.label)) +
  scale_fill_manual(name="Season", values=c("#E69F00", "#0072B2", "#00bf8b")) +
...
tiff("inulinase.species.tiff", width = 400, height = 100, units = "mm", res = 400)
ggplot(speUse.plot, aes(x=Season, y=Abundance, fill=Season)) +
  geom_dotplot(binaxis='y', stackdir = "center", dotsize = 1.5) +
...
#tiff("inulinase.species.fungi.tiff", width = 210, height = 100, units = "mm", res = 400)
ggplot(speUse2.plot, aes(x=Season, y=Abundance, fill=Season)) +
  geom_dotplot(binaxis='y', stackdir = "center", dotsize = 1.5) +
  facet_wrap(Species ~ ., ncol = 3, scales = "free") +
  scale_fill_manual(name="Season", values=c("#E69F00", "#0072B2", "#00bf8b")) +
  ylab("Relative Abundance (%)") +
