...
# Calculate residuals
ags.res <- resid(lm(Avg_Genome_Size ~ Season, data = ags))
ge.res <- resid(lm(Genome_Equivalents ~ Season, data = ags))

# Test for Normality
hist(ags.res)
qqnorm(ags.res)
qqline(ags.res)
shapiro.test(ags.res)
  # p-value = 0.4657
  # Normal
hist(ge.res)
qqnorm(ge.res)
...
TukeyHSD(aov(Avg_Genome_Size ~ Season, data = ags))
  ...
  # Winter Genomes < Summer and Spring Genomes
    # Winter has more specialists?
...
### Plot - AGS
```{r}
#tiff("averageGenomeSize.tiff", width = 4, height = 3, units = "in", res = 600)
ggplot(ags, aes(x = Season, y = AGS_kb, fill = Season)) +
  ...
  scale_y_continuous(limits = c(2620, 4000), breaks = c(2690, 2952, 3214, 3476, 3738, 4000)) +
...
# Remove outlier W20
microbe.ra <- microbe.ra[-which(rownames(microbe.ra) == "S.W20"),]
```
...
### Prepare Plot - RPKG + Microbial DNA Yield
```{r}
### Steal Season legend
season.legend <- g_legend(ggplot(exo_rpkg, aes(x=Season, y=RPKG, fill=Season)) +
...
### RPKG Plot
rpkg.ann <- data.frame(lab = c("ns", "", ""), Season = factor(c("Summer", "Winter", "Spring")))

rpkg.plot <- ggplot(exo_rpkg, aes(x=Season, y=RPKG, fill=Season)) +
  geom_dotplot(binaxis='y', stackdir='center', dotsize = 1.5) +
  scale_fill_manual(name="Season", values=c("#E69F00", "#0072B2", "#00bf8b")) +
  ylab("Exo-inulinase reads per\nkilobase per genome equivalent") +
  ...
  geom_text(data = rpkg.ann, aes(label=lab), x=c(2,0,0), y=c(0.71, 0, 0), size=4, color="black")



### Microbial DNA Yield Plot
microbeDNA.plot <- ggplot(microbe.ra, aes(x=Season, y=Microbial_DNA_Yield, fill=Season)) +
  geom_dotplot(binaxis='y', stackdir='center', dotsize = 1.5) +
  scale_fill_manual(name="Season", values=c("#E69F00", "#0072B2", "#00bf8b")) +
  ylab("Microbial DNA Yield\n(ng / mg cecal content)") +
  stat_summary(fun.data = "mean_cl_normal", geom = "pointrange",
               color = "black", position = position_dodge(0.7), size = 0.5,
               show.legend = F) +
  theme(plot.title=element_text(hjust=0.5, size=12),
        ...
        panel.grid.minor = element_blank(),
        legend.position = "none") +
  geom_text(data = rpkg.ann, aes(label=lab), x=c(2,0,0), y=c(78, 0, 0), size=4, color="black")


```


### Plot - RPKG + Microbial DNA Yield
```{r}
tiff("rpkg.microbeDNA.tiff", width = 6.5, height = 2.6, units = "in", res = 600)
grid.newpage()
pushViewport(viewport(layout=grid.layout(1,3, widths = c(1.1,1, 0.35))))

print(rpkg.plot, vp=viewport(layout.pos.row=1, layout.pos.col=1))
grid.text("A", x=unit(0.03, "npc"), y=unit(0.96, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=14,fontface="bold"))


print(microbeDNA.plot, vp=viewport(layout.pos.row=1, layout.pos.col=2))
grid.text("B", x=unit(0.03, "npc"), y=unit(0.96, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=2), gp=gpar(fontsize=14,fontface="bold"))
...
# Total dataset = 106 taxa - 1 unclassified = 105
# Winter taxa = 65 - 1 = 64
# Summer taxa = 102 -1 = 101
# Spring taxa = 98 -1 = 97
# Active taxa = 104 - 1 = 103
```
...
pwr.t2n.test(n1 = length(which(meta$Metabolism == "Active")),
             n2 = length(which(meta$Metabolism == "Hibernating")), power = 0.8)
  # d = 1.595744
...
wil.phy <- data.frame()
for(taxon in 1:dim(data.frame(otu_table(phy.phy)))[1]){
  es <- cohen.d(as.numeric(otus[taxon,]) ~ MetabOrder, hedges.correction = T)
  pwr <- pwr.t2n.test(n1 = length(which(meta$Metabolism == "Active")),
            n2 = length(which(meta$Metabolism == "Hibernating")), d = es$estimate)
  wil.phy <- rbind(wil.phy, data.frame(taxon = rownames(otus)[taxon], HedgesG = es$estimate, power = pwr[[5]]))
}
...
for(taxon in 1:dim(data.frame(otu_table(cla.phy)))[1]){
  ...
  pwr <- pwr.t2n.test(n1 = length(which(meta$Metabolism == "Active")),
            n2 = length(which(meta$Metabolism == "Hibernating")), d = es$estimate)
  wil.cla <- rbind(wil.cla, data.frame(taxon = rownames(otus)[taxon], HedgesG = es$estimate, power = pwr[[5]]))
}
...
for(taxon in 1:dim(data.frame(otu_table(ord.phy)))[1]){
  ...
  wil.ord <- rbind(wil.ord, data.frame(taxon = rownames(otus)[taxon], HedgesG = es$estimate, power = pwr[[5]]))
}
...
for(taxon in 1:dim(data.frame(otu_table(fam.phy)))[1]){
  ...
  pwr <- pwr.t2n.test(n1 = length(which(meta$Metabolism == "Active")),
            n2 = length(which(meta$Metabolism == "Hibernating")), d = es$estimate)
  wil.fam <- rbind(wil.fam, data.frame(taxon = rownames(otus)[taxon], HedgesG = es$estimate, power = pwr[[5]]))
}
...
for(taxon in 1:dim(data.frame(otu_table(gen.phy)))[1]){
  ...
  pwr <- pwr.t2n.test(n1 = length(which(meta$Metabolism == "Active")),
            n2 = length(which(meta$Metabolism == "Hibernating")), d = es$estimate)
  wil.gen <- rbind(wil.gen, data.frame(taxon = rownames(otus)[taxon], HedgesG = es$estimate, power = pwr[[5]]))
}
...
for(taxon in 1:dim(data.frame(otu_table(spe.phy)))[1]){
  ...
  pwr <- pwr.t2n.test(n1 = length(which(meta$Metabolism == "Active")),
            n2 = length(which(meta$Metabolism == "Hibernating")), d = es$estimate)
  wil.spe <- rbind(wil.spe, data.frame(taxon = rownames(otus)[taxon], HedgesG = es$estimate, power = pwr[[5]]))
}
...
tiff("inulinase.species.abund.tiff", width = 310, height = 100, units = "mm", res = 400)
