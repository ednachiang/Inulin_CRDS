...
rpkg.plot <- ggplot(exo_rpkg, aes(x=Season, y=RPKG, fill=Season)) +
  ...
  geom_text(data = rpkg.ann, aes(label=lab), x=c(2,0,0), y=c(0.71, 0, 0), size=5, color="black")
...
microbeDNA.plot <- ggplot(microbe.ra, aes(x=Season, y=Microbial_DNA_Yield, fill=Season)) +
  ...
  theme(plot.title=element_text(hjust=0.5, size=20),
        ...
        panel.grid.minor = element_blank(),
        legend.position = "none") +
  geom_text(data = rpkg.ann, aes(label=lab), x=c(2,0,0), y=c(78, 0, 0), size=5, color="black")
...
```{r}
#png("rpkg.microbeDNA.png", width = 300, height = 110, units = "mm", res = 400)
grid.newpage()
pushViewport(viewport(layout=grid.layout(1,3, widths = c(1,1, 0.25))))

print(rpkg.plot, vp=viewport(layout.pos.row=1, layout.pos.col=1))
grid.text("A", x=unit(0.03, "npc"), y=unit(0.96, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=1), gp=gpar(fontsize=26,fontface="bold"))


print(microbeDNA.plot, vp=viewport(layout.pos.row=1, layout.pos.col=2))
grid.text("B", x=unit(0.03, "npc"), y=unit(0.96, "npc"), vp=viewport(layout.pos.row=1, layout.pos.col=2), gp=gpar(fontsize=26,fontface="bold"))
