...
```{r}
coprob <- data.frame(matrix(nrow = , ncol = ))
for(m in 1:nrow(sum.ranks.use)){
  top10 <- sort(sum.ranks.use[m,], decreasing = T)[1:10]
  bot10 <- sort(sum.ranks.use[m,], decreasing = F)[1:10]
}




ace.top <- sort(sum.ranks.use[1,], decreasing = T)[1:10]
ace.bot <- sort(sum.ranks.use[1,], decreasing = F)[1:10]
```
